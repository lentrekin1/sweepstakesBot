...
while True:
    ...
    try:
        driver.get("https://www.itsourfabfashlife.com/2021/05/ice-cream-tour-2021new-york-city.html")
        ...
        WebDriverWait(driver, timeout).until(
            EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[3]/form/div[2]/a[2]"))).click()  # dana fb enter
        time.sleep(0.5)
        ...
        WebDriverWait(driver, timeout).until(
            EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div[3]/form/div[2]/a[2]"))).click()  # candace fb enter
        ...
        print(
            f'Entered with name {name} and email {email_address}, it took {end - start} seconds ({num_done} successful entry this session, {num_failed} failures in total, {num_failed/(num_done+num_failed)}% fail rate)')
    except:
        driver.quit()
        traceback.print_exc()
        num_failed += 1
